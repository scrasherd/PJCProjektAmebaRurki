...
class RandomGrowthModel : public IGrowthModel {
private:
...
public:
    ...
    void growOneStep();
    void growSingleNode(Node* parent, const std::vector<Node*>& endingNodes, const CollisionField& colField, const NodeField& nodeField, const std::vector<std::unique_ptr<Node>>& nodes, const std::vector<std::unique_ptr<Tube>>& tubes);
    float computeAngle(Node* parent);
    Node* generateAngle(float baseAngle, int generated, int direction, const NodeField& nodeField, const CollisionField& colField, const Vec2& pos);
